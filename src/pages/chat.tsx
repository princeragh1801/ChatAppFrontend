...
const MESSAGE_RECEIVED_EVENT = "ReceiveMessage";
...
const ChatPage = () => {
  ...
  const [typingChatId, setTypingChatId] = useState<string | null>(null); // To store the chat ID of the user who is typing
  /**
  ...
  const updateChatLastMessage = (
    ...
    chatToUpdate.updatedAt = message.sentAt;
  ...
  const updateChatLastMessageOnDeletion = (
    ...
    if (chatToUpdate.lastMessage === message.id) {
      requestHandler(
  ...
  const getMessages = async () => {
    ...
    //socket.send(JOIN_CHAT_EVENT, currentChat.current?.id);
  ...
  const sendChatMessage = async () => {
    ...
    socket.send(STOP_TYPING_EVENT, currentChat.current?.id);
    ...
    await requestHandler(
      ...
      async () =>
        await sendMessage(
          currentChat.current?.id || "", // Chat ID or empty string if not available
      ...
      (res) => {
        ...
        if(user != null){
          var messageData : ChatMessageInterface ={
            id: res.data,
            chat: currentChat.current?.id || "",
            content: message,
            sender: user,
            senderId: user.id,
            sentAt : new Date().toUTCString(),
            attachments: [],
            createdAt: currentChat.current?.createdAt || "",
            updatedAt: new Date().toUTCString()
          }
          setMessage(""); // Clear the message input
          setAttachedFiles([]); // Clear the list of attached files
          setMessages((prev) => [messageData, ...prev]); // Update messages in the UI
          updateChatLastMessage(currentChat.current?.id || "", messageData); // Update the last message in the chat
        }
  ...
  const handleOnMessageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      console.log("Inside handleOnMessageChange");
      ...
      console.log("Inside handleOnMessageChange after socket check");
  
      // Check if the user isn't already set as typing
      if (!selfTyping) {
        // Set the user as typing
        setSelfTyping(true);
        console.log("Typing event emitted for chat:", currentChat.current?.id);
        // Emit a typing event to the server for the current chat
        console.log("Socket : ", socket);
        socket.send(TYPING_EVENT, currentChat.current?.id);
      }
   ...
   */
  const handleOnSocketTyping = (payload: any) => {
    // console.log("Typing event received for chat:", payload); 
    // console.log("User ID: ", user?.id);
    // console.log("Sender ID: ", payload.senderId);
    if(payload.senderId === user?.id) return;
    // console.log("Sender is not user")
    // Check if the typing event is for the currently active chat.
    if (payload.chatId !== currentChat.current?.id) return;
    // console.log("Chat ID matches")
    // Set the typing state to true for the current chat.
    ToastService.info(payload.content || "Typing...");
  ...
  const handleOnSocketStopTyping = (chatId: string) => {
    ...
    console.log("Stop typing event received for chat:", chatId);
    setIsTyping(false);
  };
  ...
  /**
   * 
   * {id: 'a31bdf03-b448-42c0-43a3-08dd4a8439c4', content: 'shiowpdwdo', sentAt: '2025-02-11T10:10:20.4320881Z', sender: {…}, chatId: '66cc48fb-4b63-46c3-8e7e-08dd49caf483'}
   */
  /**
   * Handles the event when a new message is received.
   */
  const onMessageReceived = (messageData: any) => {
    if(messageData.senderId === user?.id) return;
    console.log("Message received from the user : ", messageData);
      
    // Check if the received message belongs to the currently active chat
    
    var unread : ChatMessageInterface = {
      id: messageData.id,
    ...
    if (messageData.chatId !== currentChat.current?.id) {
      // If not, update the list of unread messages
      ToastService.info(`Message received from the user : ${messageData.sender.username} content : ${messageData.content}`);
      setUnreadMessages((prev) => [unread, ...prev]);
      //alert(messageData.content);
    } else {
      // If it belongs to the current chat, update the messages list for the active chat
      setMessages((prev) => [unread, ...prev]);
    }

    // Update the last message for the chat to which the received message belongs
    updateChatLastMessage(messageData.chatId || "", unread);
  };
      ...
      <div className="w-full justify-between items-stretch h-screen flex flex-shrink-0">
        <div className="w-1/3 relative ring-white overflow-y-auto px-4">
          <div className="z-10 w-full sticky top-0 bg-dark py-4 flex justify-between items-center gap-4">
            ...
            >
              <img
              src={user?.avatar || "https://ui-avatars.com/api/?name=John+Doe"}
          ...
          {loadingChats ? (
            ...
            [...chats]
            .filter((chat) => 
                chat.name.toLocaleLowerCase().includes(localSearchQuery) 
              || (chat.user && chat.user.username.toLocaleLowerCase().includes(localSearchQuery))
            )
            ...
            <>
                ...
                <div className="flex justify-start items-center w-max gap-3">
                  {currentChat.current.isGroupChat ? (
                    <div className="w-12 relative h-12 flex-shrink-0 flex justify-start items-center flex-nowrap">
                      {currentChat.current.participants?.slice(0, 3)
                        .map((participant, i) => {
                          return (
                            <img
                              key={participant.id}
                    ...
                    <img
                      ...
                      src={ currentChat.current.user.avatar || "https://ui-avatars.com/api/?name=John+Doe"}
                     
                    />
                  )}
                  <div>
                    <p className="font-bold">
                      {currentChat.current?.isGroupChat ? currentChat.current?.name : currentChat.current?.user?.name}
                    </p>
                    <small className="text-zinc-400">
                      {currentChat.current?.isGroupChat ? currentChat.current?.name : currentChat.current?.user?.username}
                    </small>
                  </div>
                </div>
              </div>
              ...
              >
                {loadingMessages ? (
                  ...
                  <>
                    {isTyping && typingChatId === currentChat.current?.id ? <Typing /> : null}
