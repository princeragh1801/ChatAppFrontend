...
import { requestHandler, ToastService } from "../../utils";
...
const GroupChatDetailsModal: React.FC<{
  ...
  const handleGroupNameUpdate = async () => {
    ...
    if (!newGroupName) return ToastService.error("Group name is required");
    ...
    requestHandler(
      ...
      (res) => {
        ...
        ToastService.info("Group name updated to " + data.name); // Alert the user about the update.
      },
      (error) => {
        ToastService.error(error);
      } // Use default alert for any error messages.
    );
  };
  ...
  const deleteGroupChat = async () => {
    ...
    if (groupDetails?.admin !== user?.id) {
      return ToastService.error("You are not the admin of the group");
    }
    ...
    requestHandler(
      ...
      () => {
        ...
        const updatedGroupDetails = {
          ...
          participants:
            groupDetails?.participants?.filter(p => p.id !== participantId) || [],
        };
        ...
        ToastService.info("Participant removed");
      },
  ...
  const addParticipant = async () => {
    ...
    if (!participantToBeAdded)
      return ToastService.error("Please select a participant to add.");
    ...
    requestHandler(
      ...
      (res) => {
        ...
        if(data){
          ...
          const updatedGroupDetails = {
            ...groupDetails,
            participants:participant && groupDetails?.participants ? [...groupDetails?.participants, participant] : groupDetails?.participants || [],
          };
          // Update the group details state with the new details.
          setGroupDetails(updatedGroupDetails as ChatListItemInterface);
          
          // Alert the user that the participant was added.
          ToastService.info("Participant added");
          setAddingParticipant(false);
        }else{
          ToastService.error(res.errorMessage || "Something went wrong"); 
        }
        setParticipant(null);
                    ...
                    <div className="relative mt-6 flex-1 px-4 sm:px-6">
                      <div className="flex flex-col justify-center items-start">
                        <div className="flex pl-16 justify-center items-center relative w-full h-max gap-3">
                           {groupDetails?.participants && groupDetails?.participants.slice(0, 3).map((p) => {
                            return (
                              <img
                        ...
                        <div className="w-full flex flex-col justify-center items-center text-center">
                          ...
                          <p className="mt-2 text-zinc-400 text-sm">
                            Group · { groupDetails?.participants && groupDetails?.participants.length}{" "}
                            participants
                          </p>
                        </div>
                        ...
                        <div className="w-full">
                          <p className="inline-flex items-center">
                            ...
                            {groupDetails?.participants && groupDetails?.participants.length} Participants
                          </p>
                          <div className="w-full">
                                ...
                                {!addingParticipant ? (
                                  ...
                                  <div className="w-full flex justify-start items-center gap-2">
                                    <Select
                                      ...
                                      options={users
                                        .filter(user => groupDetails?.participants && !groupDetails?.participants.some(p => p.id === user.id))
                                        .map((user) => ({
                                          label: user.username,
                                          value: user.id,
                                        }))}
                                      onChange={({ value }) => {
                                        setParticipant(users.find(user => user.id === value) || null);
