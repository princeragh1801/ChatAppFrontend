...
import { classNames, requestHandler, ToastService } from "../../utils";
...
const AddChatModal: React.FC<{
  ...
  const getUsers = async () => {
    ...
    requestHandler(
      ...
      } // Use the alert as the error handler
    );
  };
  ...
  const createNewChat = async () => {
    ...
    if (!selectedUserId) return ToastService.error("Please select a user");
    ...
    await requestHandler(
      ...
      (res) => {
        ...
        if (res.statusCode === 200) {
          ToastService.error("Chat with selected user already exists");
          return;
        }else if(res.errorMessage){
          ToastService.error(res.errorMessage);
          return;
        }
  ...
  const createNewGroupChat = async () => {
    ...
    if (!groupName) return ToastService.error("Group name is required");
    // Ensure there are at least 2 group participants
    if (!groupParticipants.length || groupParticipants.length < 2)
      return ToastService.error("There must be at least 2 group participants");
